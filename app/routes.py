...
def add_sale():
    ...
    for pid, q, price, total in items_data:
        cur.execute("""INSERT INTO sale_items 
                       (sale_id, product_id, quantity, sell_price, total)
                       VALUES (%s,%s,%s,%s,%s)""",
                    (sale_id, pid, q, price, total))
...
def summary():
    ...
    # Hitung data dinamis (selalu dijalankan, untuk GET dan POST)
     # Total Pembelian
    cur.execute("SELECT quantity FROM purchases")
    result = cur.fetchone()
    if result is not None and result[0] is not None:
        total_pembelian = result[0]
    else:
        total_pembelian = 0

    # Total Penjualan
    cur.execute("SELECT sell_price FROM sale_items")
    result = cur.fetchone()
    if result is not None and result[0] is not None:
        total_penjualan = result[0]
    else:
        total_penjualan = 0


    # Ambil data summary terakhir
    cur.execute("SELECT * FROM summary ORDER BY created_at DESC LIMIT 1")
    all_summaries = cur.fetchall()

    if request.method == "POST":
        modal_awal = float(request.form["modal_awal"])

        # Perhitungan total profit baru
        total_profit = float(total_penjualan) - float(total_pembelian)

        # Simpan ke DB
        cur.execute("""
            INSERT INTO summary (modal_awal, total_pembelian, total_penjualan, total_profit)
            VALUES (%s, %s, %s, %s)
        """, (modal_awal, total_pembelian, total_penjualan, total_profit))
...
def add_modal():
    ...
    if request.method == "POST":
        ...
        cur.execute("SELECT SUM(sell_price * quantity) FROM sale_items")
...
def delete_summary(id):
    ...
    cur = mysql.connection.cursor()
    cur.execute("DELETE FROM summary WHERE id = %s", (id,))
    mysql.connection.commit()
